...
################## DEFINE ENV ##################
    ...
    for path in tqdm(trajectories):
        ...
        # Or is that captured in observations? Probably, since it seems to be a list that is appended to states
    ...
    # Gets the timesteps of entire trajectories
    # While ind >= 0 ensures we do not step out of bounds wrt. trajectories
    # timesteps + traj_lens[sorted_inds[ind]] <= num_timesteps that the len of our current trajectories
    # and our next trajectory will not overstep the total timesteps we should get
    while ind >= 0 and timesteps + traj_lens[sorted_inds[ind]] <= num_timesteps:
        timesteps += traj_lens[sorted_inds[ind]]
        num_trajectories += 1
        ind -= 1
    # Extracts the top num_trajectories from the  list of trajectories sorted low-high according to return
